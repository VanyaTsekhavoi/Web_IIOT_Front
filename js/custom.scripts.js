...
function sideNavLock() {
    if (sidenavMain.classList.contains("nav-lock")) {
        sidenavMain.querySelector(".brand-sidebar i.material-icons").innerText = "cloud_queue";
    }
    else {
        sidenavMain.querySelector(".brand-sidebar i.material-icons").innerText = "cloud";

    }
    sidenavMain.classList.toggle("nav-lock");
    main.classList.toggle("main-full");
}

sidenavMain.addEventListener("mouseenter", function (event) {
    if (!sidenavMain.classList.contains("nav-lock")) {
        sidenavMain.classList.remove("nav-collapsed");
    }
});

sidenavMain.addEventListener("mouseleave", function (event) {
    if (!sidenavMain.classList.contains("nav-lock")) {
        sidenavMain.classList.add("nav-collapsed");
    }
});

document.getElementById("slide-out").addEventListener("click", (event) => {
...
document.getElementById("journeyRolling").addEventListener('click', function (event) {
    ...
    iframe.innerHTML = '<p>x</p><iframe width="260" height="195" src="https://www.youtube.com/embed/' +
        videoId + '?rel=0&autoplay=1&start=1&controls=0&disablekb=1" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe>';
    video = iframe.children[1];
    ...
    document.querySelector("span[data-i18n = 'Warning']").innerText = "I warned you!"
    ...
    documentClickJourney = function () {
        ...
        if (video.parentNode.parentNode.parentNode.parentNode.classList.contains("active")) {
            ...
            document.getElementsByTagName("iframe")[0].requestFullscreen().catch(err => {
    ...
    document.addEventListener('click', documentClickJourney);
...
function navigationCollapse() {
    if (!$(".sidenav-main.nav-collapsible").hasClass("nav-lock")) {
        ...
        $(".sidenav-main.nav-collapsible, .navbar .nav-collapsible").addClass("nav-collapsed").removeClass("nav-expanded"),
            $("#slide-out > li.open > a").parent().addClass("close").removeClass("open"), setTimeout(
                function () {
                    if (1 < n) { var e = $(".sidenav-main .collapsible"); M.Collapsible.getInstance(e).close($(".collapsible .close").index()) }
                }, 100)
    }
}

function slideOutChat() {
    var e = $(".search").val(); if ("" != e) {
...
defaultMenuCollapse(),
    $(window).on("load", function () { $("body").removeClass("preload-transitions") }),
    $(function () {
        ...
        $(".tabs").tabs(), $(".datepicker").datepicker({
            autoClose: !0, format: "dd/mm/yyyy", container: "body", onDraw: function () {
            ...
            $("#tabs-swipe-demo").length && $("#tabs-swipe-demo").tabs({ swipeable: !0 }), $("select").formSelect();
        var n = document.getElementById("indeterminate-checkbox"); null !== n && (n.indeterminate = !0),
            ...
            $(".dropdown-submenu").on("mouseenter", function () {
                ...
                var n = e.find(".dropdown-content"); if (n) {
                    var a = n.offset().left, l = n.width();
            ...
            $(".dropdown-submenu").on("mouseleave", function () {
                var e = $(this); e.find(".dropdownSub-menu").dropdown("close"), e.find(".dropdown-content").removeClass("right-open"), e.find(".dropdown-content").removeClass("left-open")
            ...
            $(".scrollspy").scrollSpy(), $(".tooltipped").tooltip({ delay: 50 }); var a = document.querySelectorAll(".collapsible"); M.Collapsible.init(a);
        var l = document.querySelector(".collapsible.expandable"); M.Collapsible.init(l, { accordion: !1 }); if (breadcrumbsWrapper.attr("data-image")) {
            var o = breadcrumbsWrapper.attr("data-image"); breadcrumbsWrapper.addClass("breadcrumbs-bg-image"), breadcrumbsWrapper.css("background-image", "url(" + o + ")")
        }
        ...
        if (M.Collapsible.init(s, {
            ...
            var d = $(".sidenav-main .collapsible"); 1 < $("#slide-out > li.active").children().length && $("#slide-out > li.active > a").parent().addClass("close"),
        ...
        if (0 < i.find("a.active").length && (i.find("a.active").closest("div.collapsible-body").show(),
            ...
            t = 0 < $(".sidenav-main li a.active").parent("li.active").parent("ul.collapsible-sub").length ? $(".sidenav-main li a.active").parent("li.active")
                .parent("ul.collapsible-sub").position() : $(".sidenav-main li a.active").parent("li.active").position(), setTimeout(function () {
                    void 0 !== t && $(".sidenav-main ul").stop().animate({ scrollTop: t.top - 300 }, 300)
                }, 300),
            $(".nav-collapsible .navbar-toggler").click(function () {
                toogleMenuCollapse(), "radio_button_unchecked" == $(this).children().text() ? ($(this).children().text("radio_button_checked"),
                    sidenavMain.addClass("nav-lock"),
                    navCollapsible.addClass("sideNav-lock")) : ($(this).children().text("radio_button_unchecked"),
                        sidenavMain.removeClass("nav-lock"), navCollapsible.removeClass("sideNav-lock"))
                ...
                $(".sidenav-main.nav-collapsible").hasClass("nav-lock") ||
                    ($(".sidenav-main.nav-collapsible, .navbar .nav-collapsible").addClass("nav-expanded").removeClass("nav-collapsed"),
                        $("#slide-out > li.close > a").parent().addClass("open").removeClass("close"), setTimeout(function () {
            ...
            $(".slide-out-right-sidenav-chat").sidenav({ edge: "right" }), e())
            $(".leftside-navigation,.slide-out-right-body, .chat-body .collection, #ul-horizontal-nav").css("overflow", "scroll"); else {
            ...
            var e = $(this).val().toLowerCase(); "" != e ? $(".right-sidebar-chat .right-sidebar-chat-item").filter(function () {
                $(this).toggle(-1 < $(this).text().toLowerCase().indexOf(e))
            }) :
                $(".right-sidebar-chat .right-sidebar-chat-item").show()
        }); var r = $("#right-sidebar-nav #slide-out-chat .chat-body .collection"); function e() {
            try { return document.createEvent("TouchEvent"), !0 } catch (e) {
                return !1
            }
                ...
                !document.webkitIsFullScreen ? document.documentElement.requestFullScreen ? document.documentElement.requestFullScreen() :
                    document.documentElement.mozRequestFullScreen ? document.documentElement.mozRequestFullScreen() :
                        document.documentElement.webkitRequestFullScreen ? document.documentElement.webkitRequestFullScreen(Element.ALLOW_KEYBOARD_INPUT) :
                            document.documentElement.msRequestFullscreen && (document.msFullscreenElement ? document.msExitFullscreen() :
                                document.documentElement.msRequestFullscreen()) : document.cancelFullScreen ? document.cancelFullScreen() :
            ...
            $("#nav-mobile").css({ overflow: "auto" }), i18next.use(window.i18nextXHRBackend).init({
                debug: !1, fallbackLng: "en", backend: { loadPath: "../../../app-assets/data/locales/{{lng}}.json" }, returnObjects: !0
            },
                function (e, n) { jqueryI18next.init(i18next, $) }), $(".dropdown-language .dropdown-item").on("click",
                    function () {
                        var e = $(this); e.siblings(".selected").removeClass("selected"), e.addClass("selected");
                        var n = e.find(".flag-icon").attr("class"); $(".translation-button .flag-icon").removeClass().addClass(n);
                        var a = e.find("a").data("language"); i18next.changeLanguage(a, function (e, n) {
            ...
            0 < $("html[data-textdirection='rtl']").length && ($(".sidenav").sidenav({ edge: "right" }),
    ...
    $(window).on("resize", function () {
        $(window).width() < 994 ? sidenavMain.hasClass("nav-collapsed") && (sidenavMain.removeClass("nav-collapsed").addClass("nav-lock nav-expanded"),
            navCollapsible.removeClass("nav-collapsed").addClass("sideNav-lock")) : 993 < $(window).width() && $("body").hasClass("menu-collapse")
            && sidenavMain.hasClass("nav-lock") && (sidenavMain.removeClass("nav-lock nav-expanded").addClass("nav-collapsed"),
